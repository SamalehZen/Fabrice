          ...
          <div className="flex flex-col h-full gap-6">
            ...
            <div className="space-y-3 text-sm">
              {data.nameChangeAwareness.map((slice, index) => {
                ...
                return (
                  <div key={`q9-${slice.name}`} className="flex items-center justify-between rounded-xl border border-slate-200 dark:border-slate-800 bg-white/90 dark:bg-slate-900/60 px-3 py-2">
        ...
        <ChartCard title="Q10: Perception" subtitle="Impact des changements" className="col-span-1 md:col-span-2 lg:col-span-3 lg:row-span-2">
          <div className="flex flex-col gap-6 h-full">
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
              <article className="rounded-xl border border-slate-200 dark:border-slate-800 bg-gradient-to-b from-white to-slate-50 dark:from-slate-900 dark:to-slate-950 p-6 lg:col-span-2 flex flex-col sm:flex-row items-center gap-6 shadow-slate-200/30 dark:shadow-black/40">
                ...
                <div className="space-y-3 text-slate-600 dark:text-slate-300">
                  ...
                  <p className="text-xs text-slate-500 dark:text-slate-400">Total: {perceptionStats.perceptionTotal} réponses</p>
                </div>
              </article>
              <div className="grid grid-cols-2 sm:grid-cols-2 lg:grid-cols-1 gap-4">
                {perceptionStats.perceptionHighlights.map((highlight) => (
                  <article key={`badge-${highlight.label}`} className="rounded-xl border border-slate-200 dark:border-slate-800 px-4 py-3 bg-white/90 dark:bg-slate-900/60">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 flex-1">
              {q10DetailCharts.map((chart) => {
                ...
                return (
                  <article key={chart.title} className="rounded-xl border border-slate-200 dark:border-slate-800 bg-white/90 dark:bg-slate-900/60 p-5 flex flex-col gap-4">
                        ...
                        {chart.data.map((slice, index) => {
                          ...
                          return (
                            <div key={slice.name}>
                              ...
                              <div className="mt-1 h-1.5 w-full rounded-full bg-slate-200 dark:bg-slate-800 overflow-hidden" role="progressbar" aria-valuenow={percent} aria-valuemin={0} aria-valuemax={100}>
