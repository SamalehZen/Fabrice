...
import { Users, MapPin, Smile, Car, Download, Calendar, TrendingUp, ArrowUpRight, Check, ChevronDown, Bus, Footprints, CircleDot, ShoppingBag, Target, Star, Activity, AlertCircle } from 'lucide-react';
...
const QUESTION_META = {
  ...
  q1: { title: 'Q1 • Zones de résidence', subtitle: 'Origine géographique des visiteurs' },
  q2: { title: 'Q2 • Moyen de transport', subtitle: 'Mode d\'accès au centre commercial' },
  q3: { title: 'Q3 • Fréquence de visite', subtitle: "Rythme des achats" },
  q4: { title: 'Q4 • Motifs de visite', subtitle: 'Raisons principales de la venue' },
  q5: { title: 'Q5 • Enseignes concurrentes', subtitle: 'Magasin alimentaire le plus fréquenté (Hors Hyper)' },
  q6: { title: 'Q6 • Critères de choix', subtitle: 'Facteurs déterminants pour le choix du magasin' },
  q7: { title: 'Q7 • Satisfaction globale', subtitle: 'Évaluation générale de l\'expérience' },
  q8: { title: 'Q8 • Rayons préférés', subtitle: 'Attractivité des différents départements' },
  q9: { title: 'Q9 • Notoriété changement nom', subtitle: 'Connaissance de la nouvelle identité' },
    ...
    <div className="space-y-6 p-6">
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <div className="flex items-center gap-2 mt-0.5">
              <p className="text-sm text-slate-500 dark:text-gray-400">Synthèse globale des 10 questions</p>
              {selectedZone !== 'All' && (
          ...
          <div className="relative">
            ...
            {isFilterOpen && (
              <>
                ...
                <ul className="absolute top-full right-0 mt-2 w-52 bg-white dark:bg-dark-surface rounded-xl shadow-xl dark:shadow-black/50 border border-slate-100 dark:border-dark-border overflow-hidden z-20 py-1">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        <article className="relative overflow-hidden bg-gradient-to-br from-brand-500 to-brand-700 rounded-xl p-6 text-white shadow-lg shadow-brand-200 dark:shadow-brand-900/50 group transition-all hover:shadow-xl hover:-translate-y-1">
          <div className="absolute -right-6 -top-6 bg-white/10 w-32 h-32 rounded-full blur-2xl group-hover:bg-white/20 transition-all" />
          <div className="relative z-10">
            ...
            <p className="text-brand-100 text-xs font-medium uppercase tracking-wider mb-1">Nombre de répondants</p>
        ...
        <article className="relative overflow-hidden bg-white/90 dark:bg-dark-card/80 rounded-xl p-6 border border-slate-100 dark:border-dark-border shadow-sm shadow-slate-200/60 dark:shadow-black/40 group transition-all hover:shadow-xl hover:-translate-y-1">
          <div className="flex justify-between items-start mb-4">
            <div className="p-2 bg-green-50 dark:bg-green-500/20 rounded-xl"><Smile size={20} className="text-green-600 dark:text-green-300" /></div>
          ...
          <p className="text-slate-500 dark:text-gray-400 text-xs font-medium uppercase tracking-wider mb-1">Satisfaction Client</p>
          ...
          <div className="w-full bg-slate-100 dark:bg-dark-muted h-1.5 rounded-full mt-3 overflow-hidden">
            <div className="bg-green-500 dark:bg-green-300 h-full rounded-full" style={{ width: `${stats.satisfactionRate}%` }} />
          </div>
        </article>

        <article className="relative overflow-hidden bg-white/90 dark:bg-dark-card/80 rounded-xl p-6 border border-slate-100 dark:border-dark-border shadow-sm shadow-slate-200/60 dark:shadow-black/40 group transition-all hover:shadow-xl hover:-translate-y-1">
          <div className="flex justify-between items-start mb-4">
            <div className="p-2 bg-purple-50 dark:bg-purple-500/20 rounded-xl"><MapPin size={20} className="text-purple-600 dark:text-purple-300" /></div>
          ...
          <p className="text-slate-500 dark:text-gray-400 text-xs font-medium uppercase tracking-wider mb-1">Zone principale</p>
          <p className="text-xl font-bold text-slate-800 dark:text-white truncate" title={stats.topZone.name}>{stats.topZone.name}</p>
        </article>

        <article className="relative overflow-hidden bg-white/90 dark:bg-dark-card/80 rounded-xl p-6 border border-slate-100 dark:border-dark-border shadow-sm shadow-slate-200/60 dark:shadow-black/40 group transition-all hover:shadow-xl hover:-translate-y-1">
          <div className="flex justify-between items-start mb-4">
            <div className="p-2 bg-orange-50 dark:bg-orange-500/20 rounded-xl"><Car size={20} className="text-orange-600 dark:text-orange-300" /></div>
            <div className="text-right">
          ...
          <p className="text-slate-500 dark:text-gray-400 text-xs font-medium uppercase tracking-wider mb-1">Accès principal</p>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        ...
        <ChartCard title={QUESTION_META.q1.title} subtitle={QUESTION_META.q1.subtitle}>
          <ResponsiveContainer width="100%" height="100%">
             <BarChart data={filteredData.zones} layout="vertical" margin={{ top: 5, right: 30, left: 40, bottom: 5 }}>
        ...
        <ChartCard title={QUESTION_META.q2.title} subtitle={QUESTION_META.q2.subtitle}>
            ...
            return (
              <div className="flex flex-col gap-3 overflow-auto h-full pr-2">
                 {sortedTransport.map((transport, index) => {
                    ...
                    return (
                        <div key={transport.name} className="flex items-center gap-3 p-3 bg-slate-50 dark:bg-white/5 rounded-lg border border-slate-100 dark:border-white/10">
                            <div className={`p-2 rounded-lg bg-gradient-to-br ${config.gradient}`}>
                                <IconComp className="w-4 h-4 text-white" />
                            </div>
                            <div className="flex-1">
                                <div className="flex justify-between mb-1">
                                   <span className="text-xs font-semibold text-slate-700 dark:text-gray-200">{transport.name}</span>
                                   <span className="text-xs font-bold" style={{ color: config.color }}>{percent.toFixed(1)}%</span>
                                </div>
                                <div className="h-1.5 bg-slate-200 dark:bg-white/10 rounded-full overflow-hidden">
                                   <div className={`h-full bg-gradient-to-r ${config.gradient}`} style={{ width: `${percent}%` }} />
                                </div>
                            </div>
                        </div>
                    );
        ...
        <ChartCard title={QUESTION_META.q4.title} subtitle={QUESTION_META.q4.subtitle}>
           <ResponsiveContainer width="100%" height="100%">
             <BarChart data={filteredData.visitReason} layout="vertical" margin={{ top: 5, right: 10, left: 10, bottom: 5 }}>
               <CartesianGrid strokeDasharray="3 3" horizontal={false} stroke="#f1f5f9" />
               <XAxis type="number" hide />
               <YAxis dataKey="name" type="category" width={90} tick={{ fill: '#64748b', fontSize: 10 }} tickLine={false} axisLine={false} interval={0} />
               <Tooltip content={<CustomTooltip />} />
               <Bar dataKey="value" fill="#f59e0b" radius={[0, 4, 4, 0]} barSize={18}>
                  {filteredData.visitReason.map((_, index) => (
        ...
        <ChartCard title={QUESTION_META.q6.title} subtitle={QUESTION_META.q6.subtitle}>
           <ResponsiveContainer width="100%" height="100%">
             <RadarChart cx="50%" cy="50%" outerRadius="70%" data={filteredData.choiceReason}>
                <PolarGrid stroke="#e2e8f0" />
                <PolarAngleAxis dataKey="name" tick={{ fill: '#64748b', fontSize: 10 }} />
                <PolarRadiusAxis angle={30} domain={[0, 'auto']} tick={false} axisLine={false} />
                <Radar name="Critères" dataKey="value" stroke="#06b6d4" strokeWidth={2} fill="#06b6d4" fillOpacity={0.5} />
                <Tooltip content={<CustomTooltip />} />
             </RadarChart>
           </ResponsiveContainer>
        </ChartCard>

        {/* Row 3: Q5 (Major Full Width) */}
        <ChartCard title={QUESTION_META.q5.title} subtitle={QUESTION_META.q5.subtitle} className="md:col-span-2 lg:col-span-3 min-h-[400px]">
          <ResponsiveContainer width="100%" height="100%">
            <BarChart data={filteredData.competitors} margin={{ top: 20, right: 30, left: 20, bottom: 20 }}>
              <defs>
              ...
              <Bar dataKey="value" radius={[6, 6, 0, 0]} barSize={50}>
                {filteredData.competitors.map((entry, index) => (
                  <Cell key={`cell-${index}`} fill={entry.name.includes('Bawadi') ? 'url(#colorBarHighlight)' : '#cbd5e1'} />
                ))}
              </Bar>
            </BarChart>
          </ResponsiveContainer>
        ...
        <ChartCard title={QUESTION_META.q8.title} subtitle={QUESTION_META.q8.subtitle} className="md:col-span-2 lg:col-span-3 min-h-[400px]">
          <ResponsiveContainer width="100%" height="100%">
