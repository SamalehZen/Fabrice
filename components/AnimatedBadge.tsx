...
const AnimatedBadge: React.FC<AnimatedBadgeProps> = ({ text = 'Données en temps réel', color = '#0ea5e9', href }) => {
  const content = (
    ...
    >
      <div className="pointer-events-none absolute inset-x-0 bottom-full h-20 w-[165px]" aria-hidden="true">
        <svg className="h-full w-full" width="100%" height="100%" viewBox="0 0 50 50" fill="none">
          <g mask="url(#ml-mask-1)">
            <circle className="multiline ml-light-1" cx="0" cy="0" r="20" fill="url(#ml-white-grad)" />
          </g>
          <defs>
            <mask id="ml-mask-1">
      ...
      <div className="relative flex h-1 w-1 items-center justify-center rounded-full" style={{ backgroundColor: hexToRgba(color, 0.4) }}>
        <div className="flex h-2 w-2 animate-ping items-center justify-center rounded-full" style={{ backgroundColor: color }}>
          <div className="flex h-2 w-2 animate-ping items-center justify-center rounded-full" style={{ backgroundColor: color }} />
        </div>
        <div
